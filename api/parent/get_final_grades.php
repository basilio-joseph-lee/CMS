<?php
// /api/parent/get_final_grades.php
...
try {
    ...
    $where  = " WHERE f.student_id = ? ";
    $types  = "i";
    $params = [$student_id];

    if ($school_year_id > 0) { $where .= " AND f.school_year_id = ? "; $types .= "i"; $params[] = $school_year_id; }
    if ($subject_id     > 0) { $where .= " AND f.subject_id     = ? "; $types .= "i"; $params[] = $subject_id; }
    if ($advisory_id    > 0) { $where .= " AND f.advisory_id    = ? "; $types .= "i"; $params[] = $advisory_id; }

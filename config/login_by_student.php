...
date_default_timezone_set('Asia/Manila'); // ensure PH time zone
...
// Step 3: Cookie setup (same as face_login.php)
$HTTPS  = is_https();
...
// Step 4: Get enrollment info for school_year_id and advisory_id
$advisory_id = 0;
$school_year_id = 0;
$stmt = $conn->prepare("
...
// Step 5: Attendance check — only 1 "Present" record per student per day
$today = date('Y-m-d');
$check = $conn->prepare("SELECT attendance_id FROM attendance_records WHERE student_id=? AND DATE(`timestamp`)=? LIMIT 1");
...
if (!$found) {
  ...
  ");
  $insert->bind_param("iii", $student_id, $advisory_id, $school_year_id);
  $insert->execute();
  $insert->close();
}
...
echo json_encode([
  ...
  'attendance'  => [
    'inserted' => !$found,  // true if newly added today
    'date'     => $today,
    'status'   => 'Present'
  ]
]);

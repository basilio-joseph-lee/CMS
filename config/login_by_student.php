...
date_default_timezone_set('Asia/Manila'); // per-day guard uses PH date
...
// 2) Reset session (keep your original behavior)
...
// 3) Re-send cookie (same scope as face_login.php)
$HTTPS  = is_https();
...
// 4) Load enrollment (to avoid 0/0 “invalid classes”)
$advisory_id = 0;
$school_year_id = 0;
...
// 5) SINGLE atomic insert (one row per day, even with multiple rapid POSTs)
$today = date('Y-m-d');
$subject_id = 0; // per your instruction: not per subject

// Use INSERT ... SELECT ... WHERE NOT EXISTS to be idempotent even under concurrency
$ins = $conn->prepare("
...
$ins->execute();
$rows_added = $conn->affected_rows; // 1 if inserted, 0 if already had one
$ins->close();
...
echo json_encode([
  ...
  'attendance'  => [
    'inserted'        => ($rows_added === 1),
    'alreadyMarked'   => ($rows_added === 0),
    'date'            => $today,
    'status'          => 'Present',

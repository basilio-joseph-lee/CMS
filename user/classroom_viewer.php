...
// ---------- Inline AJAX API handler (paste after session_start() + db include) ----------
if (isset($_GET['action']) && (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')) {
  ...
  // Reuse both teacher and student session keys:
  $isTeacher = isset($_SESSION['teacher_id']);
  $isStudent = isset($_SESSION['student_id']);

  // If neither session exists, return empty but 403
  ...
  // Determine class pairing (sy/ad/sj) from session; fall back to enrollment if missing
  $sy = intval($_SESSION['school_year_id'] ?? 0);
  $ad = intval($_SESSION['advisory_id'] ?? 0);
  $sj = intval($_SESSION['subject_id'] ?? 0);

  // helper: normalize avatar paths (same as teacher code)
  ...
  // --- derive class if missing ---
  if ((!$ad || !$sj) && ($isTeacher || $isStudent)) {
    if ($isTeacher) {
      // try teacher subjects
      $q = $conn->prepare("
        ...
        LIMIT 1
      ");
      $q->bind_param("i", $_SESSION['teacher_id']);
      $q->execute();
      $q->bind_result($ad2,$sj2,$sy2);
      if ($q->fetch()) { $ad = intval($ad2); $sj = intval($sj2); $sy = intval($sy2); }
      $q->close();
    } else {
      // student: try student_enrollments
      $q = $conn->prepare("
    ...
    // persist derived values in session so the UI and subsequent calls see them
    $_SESSION['school_year_id'] = $sy;
    $_SESSION['advisory_id']    = $ad;
    $_SESSION['subject_id']     = $sj;
  }

  // if we still don't have a proper class, return empty results (but 200)
  if (!$sy || !$ad || !$sj) {
...
const [S,P] = await Promise.all([
  ...
  fetch(SELF_API + 'get_seating',  { ...FETCH_OPTS }).then(r=>r.json())
    ...
    async function refreshStudents(){
      try{
const S=await fetch(SELF_API + 'get_students', { ...FETCH_OPTS }).then(r=>r.json());
...
const isAway           = isBackOverride ? false : (overlayApplies || individuallyAway);
        ...
        node.innerHTML = `
          <div class="card ${has-student?'has-student':'empty'} ${isAway?'is-away':''}">
            ${hasStudent ? `

<?php
/**
 * select_subject.php (STUDENT-ONLY, root)
 * - Shows subjects for active school year (for the logged-in student)
 * - When a subject is clicked (via GET), saves context to session
 * - Auto-logs attendance based on schedule_timeblocks when appropriate
 */
...
if (!isset($_SESSION['student_id'])) {
    header('Location: /CMS/user/face_login.php');
    exit;
}
if ($role !== 'STUDENT') {
    $_SESSION['role'] = 'STUDENT'; // auto-promote role if missing
}
...
//    then (student) auto-log attendance and redirect to student dashboard.
// ======================================================================
if (
    isset(
        $_GET['subject_id'],
    ...
    // Student always goes to the student dashboard
session_write_close();
header("Location: /CMS/user/dashboard.php?new_login=1");
exit;
}

// ======================================================================
// 2) Load active school year and list of subjects for chooser (student)
// ======================================================================
$active_sy_id = null;
$current_year = '';
$subjects = [];
...
<head>
  ...
  <style>
    body {
      /* root path: /CMS/ → background in /CMS/img/ */
...
<body class="min-h-screen flex flex-col items-center px-4 py-8">
  ...
  <?php elseif ($active_sy_id && count($subjects) === 0): ?>
    <div class="bg-white/90 rounded-xl p-6 shadow max-w-xl text-center">
      ...
      <p class="text-sm text-gray-600 mt-2">Make sure subjects are assigned to the student for this school year.</p>
    </div>
  <?php else: ?>
    <div class="bg-white/90 rounded-xl p-6 shadow max-w-xl text-center">

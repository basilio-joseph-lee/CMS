...
<style>
  ...
  .desk-rect{
    ...
    background:linear-gradient(180deg,var(--desk-grad-1) 0%,var(--desk-grad-2) 100%);
    border:2px solid var(--desk-border);
    border-radius:6px 6px 2px 2px; margin:0 auto; position:relative; z-index:1;
  }
  .desk-rect::before,.desk-rect::after{
    content:""; position:absolute; width:6px; height:28px; background:var(--leg); bottom:-28px;
  }
  .desk-rect::before{ left:10px; } .desk-rect::after{ right:10px; }

  /* BASE CHAIR (dimensioned by variables) */
  .chair-back{
    width:var(--back-w); height:var(--back-h); background:var(--chair-back);
    border:2px solid var(--chair-border); border-radius:var(--back-r); margin:0 auto; position:relative;
  }
  .chair-seat{
    width:var(--seat-w); height:var(--seat-h); background:var(--chair-seat);
    border:2px solid var(--chair-border); border-radius:var(--seat-r);
    margin:var(--seat-mt) auto 0; position:relative; z-index:0;
  }

  /* SHAPE EXTRAS (composable) */
  #stage.no-back .chair-back{ display:none; }
  #stage.extra-tablet .chair-seat::after{
    content:""; position:absolute; right:-18px; top:-10px; width:28px; height:16px;
    background:var(--desk-grad-1); border:2px solid var(--desk-border); border-radius:5px;
  }
  #stage.extra-post .chair-seat::after{
    content:""; position:absolute; left:50%; transform:translateX(-50%); bottom:-22px;
    width:6px; height:24px; background:var(--chair-border); border-radius:3px;
  }
  ...
  #stage.extra-wings .chair-back::after{
    content:""; position:absolute; top:4px; width:10px; height:18px; background:var(--chair-back);
    border:2px solid var(--chair-border); border-radius:8px;
  }
  ...
  #stage.extra-stripes .chair-back{
    background:
      repeating-linear-gradient(90deg, rgba(0,0,0,.10) 0 6px, rgba(255,255,255,.12) 6px 12px),
      var(--chair-back);
  }
  #stage.extra-notch .chair-seat::after{
    content:""; position:absolute; left:50%; transform:translateX(-50%); top:-6px;
    width:14px; height:8px; background:var(--chair-seat); border:2px solid var(--chair-border); border-bottom:none; border-radius:10px 10px 0 0;
  }
  #stage.extra-splitback .chair-back{
    background:transparent; border-color:transparent; height:0;
  }
  #stage.extra-splitback .chair-back::before,
  #stage.extra-splitback .chair-back::after{
    content:""; position:absolute; top:-22px; width:30px; height:20px; background:var(--chair-back);
    border:2px solid var(--chair-border); border-radius:4px;
  }
  ...
  .avatar-wrapper{
    ...
    width:60px; height:60px; z-index:2; transform-style: preserve-3d;
  }
  .avatar-bias{ width:100%; height:100%; transform:none; }
.avatar-img{
    width:100%; height:100%; object-fit:contain; display:block;
    animation: head-tilt var(--headDur, 2.8s) ease-in-out infinite alternate;
...
.seat .name{
    margin-top:6px; font-size:12px; text-align:center; font-weight:700; color:#1f2937;
    text-shadow: 0 1px 0 rgba(255,255,255,.9), 0 0 2px rgba(0,0,0,.08); pointer-events:none;
}
  ...
  .status-bubble{
    ...
    box-shadow:0 2px 4px rgba(0,0,0,.2); opacity:1; pointer-events:none;
  }
  .status-bubble::before,.status-bubble::after{
    content:""; position:absolute; background:#fff; border:2px solid #111; border-radius:50%;
  }
    ...
    <div class="tab-body" data-tab-panel="layouts">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="tpl" data-layout="pairs">
          <img src="data:image/svg+xml;utf8,<?php
            echo rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="360" height="220">
              <rect width="100%" height="100%" fill="#f8fafc"/>
              <rect x="24" y="18" width="110" height="32" rx="6" fill="#60a5fa"/>
              <text x="79" y="38" text-anchor="middle" font-size="16" fill="#fff" font-weight="700">Teacher</text>
              <g fill="#06b6d4">
                <rect x="60" y="80" width="24" height="18" rx="3"/>
                <rect x="90" y="80" width="24" height="18" rx="3"/>
                <rect x="60" y="112" width="24" height="18" rx="3"/>
                <rect x="90" y="112" width="24" height="18" rx="3"/>
                <rect x="200" y="80" width="24" height="18" rx="3"/>
                <rect x="230" y="80" width="24" height="18" rx="3"/>
                <rect x="200" y="112" width="24" height="18" rx="3"/>
                <rect x="230" y="112" width="24" height="18" rx="3"/>
              </g>
            </svg>');
          ?>" alt="Pairs">
        ...
        <div class="tpl" data-layout="grid">
          <img src="data:image/svg+xml;utf8,<?php
            ...
            $gridRects = '';
            foreach ($rows as $y) {
              foreach ([60,100,140,180,220] as $x) {
                $gridRects .= "<rect x='$x' y='$y' width='24' height='18' rx='3'/>";
              }
            }
            echo rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="360" height="220">
              <rect width="100%" height="100%" fill="#f8fafc"/>
        ...
        <div class="tpl" data-layout="rows">
          <img src="data:image/svg+xml;utf8,<?php
            ...
            $rowRects = '';
            foreach ($rows as $y) {
              foreach ([60,100,140,180,220,260,300,340] as $x) {
                $rowRects .= "<rect x='$x' y='$y' width='24' height='18' rx='3'/>";
              }
            }
            echo rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="360" height="220">
              <rect width="100%" height="100%" fill="#f8fafc"/>
        ...
        <div class="tpl" data-layout="g4">
          <img src="data:image/svg+xml;utf8,<?php
            $blocks=''; $centers=[[100,100],[200,100],[300,100],[100,160],[200,160],[300,160]];
            foreach($centers as [$cx,$cy]){
              foreach([[ -22,-18],[0,-18],[-22,4],[0,4]] as $d){
                $blocks.="<rect x='".($cx+$d[0])."' y='".($cy+$d[1])."' width='20' height='16' rx='3'/>";
              }
            }
            echo rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="360" height="220">
              <rect width="100%" height="100%" fill="#f8fafc"/>
        ...
        <div class="tpl" data-layout="ushape">
          <img src="data:image/svg+xml;utf8,<?php
            $bottom=''; foreach([60,100,140,180,220,260,300] as $x){ $bottom.="<rect x='$x' y='168' width='24' height='18' rx='3'/>"; }
            $sides = "<rect x='48' y='116' width='24' height='18' rx='3'/><rect x='320' y='116' width='24' height='18' rx='3'/>".
                     "<rect x='48' y='88' width='24' height='18' rx='3'/><rect x='320' y='88' width='24' height='18' rx='3'/>".
                     "<rect x='48' y='60' width='24' height='18' rx='3'/><rect x='320' y='60' width='24' height='18' rx='3'/>";
            echo rawurlencode('<svg xmlns="http://www.w3.org/2000/svg" width="360" height="220">
              <rect width="100%" height="100%" fill="#f8fafc"/>
              <rect x="24" y="18" width="110" height="32" rx="6" fill="#60a5fa"/>
              <text x="79" y="38" text-anchor="middle" font-size="16" fill="#fff" font-weight="700">Teacher</text>
              <g fill="#06b6d4">'.$bottom.$sides.'</g>
            </svg>');
          ?>" alt="U Shape">
...
const ACTION_LABELS = {
  ...
  participated:'Participated', help_request:'Needs Help', attendance:'Attendance', out_time:'Out Time'
};
...
function placeGridCentered(cols, rows, totalChairs, topOffset=0){
  ...
  const pos=[]; for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ pos.push({x:startX+c*(M.seatW+M.gapX), y:startY+r*(M.seatH+M.gapY)}); if(pos.length===totalChairs) return pos; } }
  return pos;
}
function placeRows(n=30){return placeGridCentered(6,5,n);}
function placePairs(n=12){return placeGridCentered(6,2,n);}
function placeGrid25(n=25){return placeGridCentered(5,5,n);}
function placeGroupsOfFour(n=24){
  ...
  const pos=[]; for(let gy=0;gy<groupsY;gy++){ for(let gx=0;gx<groupsX;gx++){ const baseX=startX+gx*(groupSeatW+interGapX), baseY=startY+gy*(groupSeatH+interGapY);
    const cells=[{x:baseX,y:baseY},{x:baseX+(M.seatW+M.gapX),y:baseY},{x:baseX,y:baseY+(M.seatH+M.gapY)},{x:baseX+(M.seatW+M.gapX),y:baseY+(M.seatH+M.gapY)}];
    for(const c of cells){ pos.push(c); if(pos.length===n) return pos; }
  }} return pos;
}
function placeUShape(bottomWanted=10, sideRowsWanted=3, n=16){
  const M=stageMetrics(), bottom=Math.max(3,Math.min(bottomWanted,M.maxCols)), totalBottomW=bottom*M.seatW+(bottom-1)*M.gapX;
  const startX=Math.max(M.pad,Math.round((M.rectW-totalBottomW)/2)), bottomY=Math.max(M.pad,M.rectH-M.pad-M.seatH);
  const pos=[]; for(let c=0;c<bottom && pos.length<n;c++){ pos.push({x:startX+c*(M.seatW+M.gapX), y:bottomY}); }
  const sideRows=Math.max(1,Math.min(sideRowsWanted,M.maxRows-1));
  const leftX=startX, rightX=startX+(bottom-1)*(M.seatW+M.gapX), topMostY=Math.max(M.pad,bottomY-sideRows*(M.seatH+M.gapY));
  for(let r=0;r<sideRows && pos.length<n;r++){ const y=bottomY-(r+1)*(M.seatH+M.gapY); pos.push({x:leftX,y:Math.max(topMostY,y)}); if(pos.length<n) pos.push({x:rightX,y:Math.max(topMostY,y)}); }
  return pos;
}

/* ---------- Data loading & roster sync ---------- */
async function loadData(){
  const [S,P]=await Promise.all([fetch('../../api/get_students.php').then(r=>r.json()), fetch('../../api/get_seating.php').then(r=>r.json())]);
  students=S.students||[]; seats=normalizeSeating(P.seating);
...
function normalizeSeating(raw){
  ...
  if(Array.isArray(raw)){
    return raw.map((r,i)=>({ seat_no:parseInt(r.seat_no||i+1,10),
      student_id:(r.student_id!==null && r.student_id!=='')?parseInt(r.student_id,10):null,
      x:(r.x!=null)?parseFloat(r.x):null, y:(r.y!=null)?parseFloat(r.y):null }));
  }
  const arr=[]; Object.keys(raw).forEach((k,i)=>arr.push({seat_no:parseInt(k,10),student_id:raw[k]?parseInt(raw[k],10):null,x:null,y:null}));
...
function autoAssignUnseated(){
  ...
  students.forEach(stu=>{
    if(!seatedIds.has(stu.student_id)){
      let slot=empties()[0];
      if(!slot){
        // add a new empty chair position and use it
      ...
      slot.student_id=stu.student_id; seatedIds.add(stu.student_id);
    }
  });
}
/* poll roster to auto-seat brand new students */
...
function renderSeats(){
  seatLayer.innerHTML='';
  const M=stageMetrics();
  seats=seats.map((s,i)=>{ if(s.x==null || s.y==null){ const c=i%Math.max(1,M.maxCols), r=Math.floor(i/Math.max(1,M.maxCols)); s.x=M.pad+c*(M.seatW+M.gapX); s.y=M.pad+r*(M.seatH+M.gapY);} return s; });
  const colsForBias=Math.max(1,M.maxCols);

  seats.forEach((seat,i)=>{
    const node=document.createElement('div'); node.className='seat draggable';
    node.dataset.seatNo=seat.seat_no; node.dataset.studentId=seat.student_id ?? '';
...
${(()=>{
  ...
  bindMenus(); enableDragging();
}
function renderStats(){
  const total=students.length;
  const seated=seats.reduce((a,s)=>a+(s.student_id!=null?1:0),0);
  document.getElementById('stats').textContent=`Students: ${total} • Seated: ${seated} • Chairs: ${seats.length}`;
...
async function refreshBehavior(){
  try{
    ...
    behaviorMap={};
    if(res.map && typeof res.map==='object'){ behaviorMap=res.map; }
    else if(Array.isArray(res)){ res.forEach(row=>{
...
function bindMenus(){
  seatLayer.querySelectorAll('.seat').forEach(card=>{
    card.addEventListener('contextmenu', e=>{
      ...
      menu.style.left=(e.pageX+2)+'px'; menu.style.top=(e.pageY+2)+'px';
  ...
  menu.querySelectorAll('[data-action-student]').forEach(btn=>{
    btn.onclick = async ()=>{
      ...
      const r=await fetch('../../api/mark_attendance.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({student_id:sid,status})}).then(r=>r.json());
      toast(r.message||'Saved'); menu.classList.add('hidden');
  ...
  menu.querySelectorAll('[data-log]').forEach(btn=>{
    btn.onclick = async ()=>{
      ...
      const r=await fetch('../../api/log_behavior.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({student_id:sid,action_type})}).then(r=>r.json()).catch(()=>({message:'Network error'}));
      toast(r.message||'Logged'); menu.classList.add('hidden');
...
function enableDragging(){
  ...
  function attachDrag(el,onDrop){
    ...
    function move(e){ if(!dragging) return; const p=(e.touches?e.touches[0]:e); const sRect=stage.getBoundingClientRect(); const r=el.getBoundingClientRect();
      let left=p.clientX-sRect.left-dX, top=p.clientY-sRect.top-dY; left=clamp(left,0,sRect.width-r.width); top=clamp(top,0,sRect.height-r.height); el.style.left=left+'px'; el.style.top=top+'px'; }
  ...
  seatLayer.querySelectorAll('.seat').forEach(el=>{
    attachDrag(el,()=>{ const no=parseInt(el.dataset.seatNo,10); const seat=seats.find(s=>s.seat_no===no);
...
/* ---------- THEMES & SHAPES ---------- */
const THEMES=[{id:'classic',name:'Classic Wood',sw:'#d2a86a',d1:'#e6cfa7',d2:'#d2a86a',db:'#a16a2a',leg:'#8b5e34',seat:'#d1d5db',back:'#9ca3af',cb:'#6b7280'},{id:'ocean',name:'Ocean Blue',sw:'#4fc3f7',d1:'#b3e5fc',d2:'#4fc3f7',db:'#0288d1',leg:'#01579b',seat:'#b2f5ea',back:'#80deea',cb:'#0ea5e9'},{id:'spring',name:'Spring Green',sw:'#86efac',d1:'#d9f99d',d2:'#86efac',db:'#22c55e',leg:'#166534',seat:'#bbf7d0',back:'#86efac',cb:'#16a34a'},{id:'berry',name:'Berry Pop',sw:'#e879f9',d1:'#f5d0fe',d2:'#e879f9',db:'#a21caf',leg:'#6b21a8',seat:'#fde68a',back:'#fbcfe8',cb:'#be185d'},{id:'modern',name:'Modern Gray',sw:'#cbd5e1',d1:'#e5e7eb',d2:'#cbd5e1',db:'#475569',leg:'#334155',seat:'#e2e8f0',back:'#cbd5e1',cb:'#334155'},{id:'sunset',name:'Sunset',sw:'#fb923c',d1:'#fed7aa',d2:'#fb923c',db:'#f59e0b',leg:'#92400e',seat:'#fee2e2',back:'#fecaca',cb:'#ea580c'},{id:'jungle',name:'Jungle',sw:'#16a34a',d1:'#86efac',d2:'#22c55e',db:'#15803d',leg:'#065f46',seat:'#bbf7d0',back:'#4ade80',cb:'#166534'},{id:'lavender',name:'Lavender',sw:'#a78bfa',d1:'#ede9fe',d2:'#a78bfa',db:'#7c3aed',leg:'#5b21b6',seat:'#f5f3ff',back:'#ddd6fe',cb:'#6d28d9'},{id:'cocoa',name:'Cocoa',sw:'#a16207',d1:'#fde68a',d2:'#f59e0b',db:'#a16207',leg:'#7c2d12',seat:'#fef3c7',back:'#fcd34d',cb:'#92400e'},{id:'slate',name:'Slate',sw:'#64748b',d1:'#cbd5e1',d2:'#94a3b8',db:'#475569',leg:'#334155',seat:'#e2e8f0',back:'#94a3b8',cb:'#1f2937'},{id:'mint',name:'Mint',sw:'#34d399',d1:'#d1fae5',d2:'#34d399',db:'#10b981',leg:'#065f46',seat:'#ecfeff',back:'#a7f3d0',cb:'#0f766e'},{id:'coral',name:'Coral',sw:'#fb7185',d1:'#fecdd3',d2:'#fb7185',db:'#f43f5e',leg:'#9f1239',seat:'#ffe4e6',back:'#fecdd3',cb:'#be123c'},{id:'sky',name:'Sky',sw:'#38bdf8',d1:'#e0f2fe',d2:'#38bdf8',db:'#0284c7',leg:'#075985',seat:'#e0f2fe',back:'#bae6fd',cb:'#0369a1'},{id:'lemon',name:'Lemon',sw:'#fde047',d1:'#fef9c3',d2:'#fde047',db:'#ca8a04',leg:'#854d0e',seat:'#fefce8',back:'#fde68a',cb:'#a16207'},{id:'rose',name:'Rose',sw:'#f472b6',d1:'#fce7f3',d2:'#f472b6',db:'#db2777',leg:'#9d174d',seat:'#ffe4e6',back:'#f9a8d4',cb:'#be185d'},{id:'night',name:'Night',sw:'#0ea5e9',d1:'#1f2937',d2:'#0ea5e9',db:'#0c4a6e',leg:'#082f49',seat:'#cbd5e1',back:'#94a3b8',cb:'#0f172a'},{id:'sand',name:'Sand',sw:'#facc15',d1:'#fde68a',d2:'#facc15',db:'#ca8a04',leg:'#92400e',seat:'#fef3c7',back:'#fde68a',cb:'#a16207'},{id:'copper',name:'Copper',sw:'#ea580c',d1:'#fed7aa',d2:'#fdba74',db:'#c2410c',leg:'#7c2d12',seat:'#fff7ed',back:'#fed7aa',cb:'#7c2d12'},{id:'denim',name:'Denim',sw:'#2563eb',d1:'#bfdbfe',d2:'#60a5fa',db:'#1d4ed8',leg:'#1e3a8a',seat:'#e0e7ff',back:'#bfdbfe',cb:'#1e40af'},{id:'lime',name:'Lime',sw:'#84cc16',d1:'#ecfccb',d2:'#a3e635',db:'#65a30d',leg:'#3f6212',seat:'#f7fee7',back:'#d9f99d',cb:'#4d7c0f'}];
const SHAPES=[{id:'classic',name:'Classic',v:{bw:'70px',bh:'28px',br:'4px',sw:'70px',sh:'18px',sr:'4px',sm:'-6px'},ex:[]},{id:'rounded',name:'Rounded Plastic',v:{bw:'72px',bh:'34px',br:'18px',sw:'76px',sh:'22px',sr:'9999px',sm:'-4px'},ex:[]},{id:'stool',name:'Stool',v:{bw:'0px',bh:'0px',br:'0',sw:'42px',sh:'42px',sr:'9999px',sm:'6px'},ex:['no-back','extra-post']},{id:'tablet',name:'Tablet Arm',v:{bw:'64px',bh:'24px',br:'6px',sw:'68px',sh:'18px',sr:'10px',sm:'-4px'},ex:['extra-tablet']},{id:'bench',name:'Bench',v:{bw:'92px',bh:'22px',br:'6px',sw:'92px',sh:'16px',sr:'6px',sm:'-6px'},ex:[]},{id:'highback',name:'High Back',v:{bw:'70px',bh:'44px',br:'10px',sw:'70px',sh:'18px',sr:'6px',sm:'-6px'},ex:[]},{id:'split',name:'Split Back',v:{bw:'72px',bh:'22px',br:'6px',sw:'70px',sh:'18px',sr:'6px',sm:'-6px'},ex:['extra-splitback']},{id:'winged',name:'Winged',v:{bw:'70px',bh:'30px',br:'10px',sw:'70px',sh:'18px',sr:'8px',sm:'-6px'},ex:['extra-wings']},{id:'saddle',name:'Saddle Seat',v:{bw:'68px',bh:'26px',br:'8px',sw:'72px',sh:'20px',sr:'16px',sm:'-6px'},ex:['extra-notch']},{id:'bubble',name:'Bubble',v:{bw:'60px',bh:'38px',br:'9999px',sw:'58px',sh:'18px',sr:'9999px',sm:'-8px'},ex:[]},{id:'square',name:'Square',v:{bw:'70px',bh:'28px',br:'2px',sw:'70px',sh:'18px',sr:'2px',sm:'-6px'},ex:[]},{id:'trape',name:'Trapezoid Seat',v:{bw:'68px',bh:'26px',br:'8px',sw:'78px',sh:'18px',sr:'12px',sm:'-6px'},ex:[]},{id:'arc',name:'Arc Back',v:{bw:'76px',bh:'30px',br:'9999px 9999px 6px 6px',sw:'70px',sh:'18px',sr:'8px',sm:'-6px'},ex:[]},{id:'lattice',name:'Lattice Back',v:{bw:'70px',bh:'30px',br:'8px',sw:'70px',sh:'18px',sr:'8px',sm:'-6px'},ex:['extra-stripes']},{id:'oval',name:'Oval Back',v:{bw:'76px',bh:'28px',br:'9999px / 60% 60% 30% 30%',sw:'70px',sh:'18px',sr:'10px',sm:'-6px'},ex:[]},{id:'bucket',name:'Bucket',v:{bw:'70px',bh:'28px',br:'12px',sw:'70px',sh:'20px',sr:'14px 14px 10px 10px',sm:'-6px'},ex:[]},{id:'slim',name:'Slim Back',v:{bw:'56px',bh:'36px',br:'12px',sw:'68px',sh:'18px',sr:'10px',sm:'-6px'},ex:[]},{id:'wide',name:'Wide Back',v:{bw:'100px',bh:'20px',br:'8px',sw:'90px',sh:'16px',sr:'8px',sm:'-6px'},ex:[]},{id:'flip',name:'Low Seat',v:{bw:'70px',bh:'28px',br:'8px',sw:'68px',sh:'14px',sr:'10px',sm:'-10px'},ex:[]},{id:'tripod',name:'Tripod Stool',v:{bw:'0px',bh:'0px',br:'0',sw:'44px',sh:'44px',sr:'9999px',sm:'4px'},ex:['no-back','extra-tripod']}];

function applyColorTheme(id){ const t=THEMES.find(x=>x.id===id)||THEMES[0];
  stage.style.setProperty('--desk-grad-1',t.d1); stage.style.setProperty('--desk-grad-2',t.d2); stage.style.setProperty('--desk-border',t.db);
  stage.style.setProperty('--leg',t.leg); stage.style.setProperty('--chair-seat',t.seat); stage.style.setProperty('--chair-back',t.back); stage.style.setProperty('--chair-border',t.cb);
  localStorage.setItem(COLOR_KEY,id); document.querySelectorAll('#colorList .style-btn').forEach(b=>b.classList.toggle('active',b.dataset.theme===id));
}
...
function applyChairShape(id){ const s=SHAPES.find(x=>x.id===id)||SHAPES[0], v=s.v;
  ...
  localStorage.setItem(SHAPE_KEY,id); document.querySelectorAll('#shapeList .style-btn').forEach(b=>b.classList.toggle('active',b.dataset.shape===id));
}
function buildCustomizerLists(){
  const colorList=document.getElementById('colorList'); colorList.innerHTML='';
  THEMES.forEach(t=>{ const btn=document.createElement('button'); btn.className='style-btn'; btn.dataset.theme=t.id;
    btn.innerHTML=`<span class="swatch" style="background:linear-gradient(180deg,${t.d1},${t.d2});"></span><span>${t.name}</span>`;
    btn.onclick=()=>applyColorTheme(t.id); colorList.appendChild(btn); });
  const shapeList=document.getElementById('shapeList'); shapeList.innerHTML='';
  SHAPES.forEach(s=>{ const btn=document.createElement('button'); btn.className='style-btn'; btn.dataset.shape=s.id; btn.textContent=s.name; btn.onclick=()=>applyChairShape(s.id); shapeList.appendChild(btn); });
  applyColorTheme(localStorage.getItem(COLOR_KEY)||'classic'); applyChairShape(localStorage.getItem(SHAPE_KEY)||'classic');
}
...
document.querySelectorAll('.tab-btn').forEach(btn=>{
  btn.addEventListener('click',()=>{ document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active');
    const tab=btn.dataset.tab; document.querySelectorAll('.tab-body').forEach(p=>p.classList.toggle('hidden',p.dataset.tabPanel!==tab)); });
});
document.querySelectorAll('.tpl').forEach(t=>{ t.onclick=()=>{ applyLayout(t.dataset.layout); tplModal.classList.remove('open'); }; });
